## 通用 (General Etiquette)

- 优先保证代码简洁易懂。请记住，代码是写给人看的，只是机器恰好可以运行 。(code is written for human, it happens to run on machines.)
- 别搞过度设计，简单实用就好。
- 写代码时，要注意圈复杂度，函数尽量小，尽量可以复用，尽量不写重复代码。
- 写代码时，注意模块设计，尽量使用设计模式。
- 给我解释代码的时候，说人话，别拽专业术语。最好有图（mermaid 风格）
- 帮我实现的时候，需要给出原理，并给出执行步骤，最好有图（mermaid 风格）
